...
The native path applies to **every** `[[database]]` that has a `[native.database.<name>]` wiring,
one after another, matching the Cloudflare path's one-`wrangler`-call-per-database behaviour.
