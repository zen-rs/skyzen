...
impl WebSocket {
    pub(crate) async fn from_raw_socket<IO, R>(_stream: IO, _role: R, config: Option<WebSocketConfig>) -> Self {
        Self {
    ...
    pub async fn send(&mut self, _message: WebSocketMessage) -> WebSocketResult<()> {
    ...
    pub async fn close(&mut self, _close_frame: Option<WebSocketCloseFrame>) -> WebSocketResult<()> {
        Err(unsupported_error())
    }
}
